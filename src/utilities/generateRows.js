...
function generateForestMetadata() {
    ...
    const trees = Array.from({length: Math.floor(Math.random() * 4) + 2}, () => {
        let tileIndex;
        do {
...
function generateCarLaneMetadata() {
    ...
    const speed = randomElement([125, 150, 180]);

    const occupiedTiles = new Set();

    const vehicles = Array.from({length: Math.floor(Math.random() * 2) + 2}, () => {
